...
import { WORKER_ID, WORKER_TYPE, DEF_PROXY, LISTENERS, DEF_PROXY_PORT, DEF_PROXY_UN, DEF_PROXY_PW } from './constants.js'
            ...
            if (request.redirectChain().length === 0 && response.ok()) {
                ...
                try {
                    ...
                    if (responses.size == (["fr"].includes(data.country) ? 4 : 3)) {
                        ...
                        await axios.post(
                            `http://localhost:8000/internal/slotUpdateV2/${c}`,
...
async function bg_task_tls_adv() {
    ...
    var resp = await axios.post(
        "http://localhost:8000/internal/getListenerData",
        {
